...
**v1.1 の変更点 / What's new in v1.1**

- 🌐 JP / EN 言語切替ボタンを追加 / Added JP / EN language toggle
- 🔵 軸対称モデルの 3D 最終形状表示（ホールケーキ表示） / 3D final-shape view for axisymmetric models (cake-slice style)
- 🔧 工具応力を修正（GMSH を EXE に同梱）/ Fixed tool stress (GMSH now bundled in EXE)
  - v1.0 では GMSH を除外していたため工具応力が表示されなかった不具合を修正
  - v1.0 excluded GMSH, causing tool stress to silently fail — now fixed
